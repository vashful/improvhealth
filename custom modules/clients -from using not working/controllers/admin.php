...
/**
 * Admin controller for the clients module
 *
...
class Admin extends Admin_Controller {
	...
	private $validation_rules = array(
		array(
			'field' => 'first_name',
			'label' => 'lang:client_first_name_label',
			'rules' => 'required|utf8'
		),
		array(
			'field' => 'last_name',
			'label' => 'lang:client_last_name_label',
			'rules' => 'utf8'
		),
		array(
			'field' => 'email',
			'label' => 'lang:client_email_label',
			'rules' => 'required|valid_email'
		),
		array(
			'field' => 'password',
			'label' => 'lang:client_password_label',
		...
		array(
			'field' => 'display_name',
			'label' => 'lang:client_display_name',
			'rules' => 'min_length[3]|max_length[50]'
		),
		array(
			'field' => 'group_id',
			'label' => 'lang:client_group_label',
			'rules' => 'required|callback__group_check'
		),
		array(
			'field' => 'active',
			'label' => 'lang:client_active_label',
			'rules' => ''
		)
	);
	...
	public function index()
	{
		...
		$pagination = create_pagination('admin/clients/index', $this->client_m->count_by($base_where));


		// Using this data, get the relevant results
		$clients = $this->client_m
	...
	public function create()
	{
		...
		if ($this->form_validation->run() !== FALSE)
		{
			...
			if ($client_id = $this->ion_auth->register($username, $password, $email, $client_data, $group->name))
			{
		...
		$this->template
				->title($this->module_details['name'], lang('client_add_title'))
	...
	public function edit($id = 0)
	{
		...
		if (!$member)
		{
			$this->session->set_flashdata('error', $this->lang->line('client_edit_client_not_found_error'));
			redirect('admin/clients');
		}
		...
		$this->data->member = & $member;
		$this->template
			->title($this->module_details['name'], sprintf(lang('client_edit_title'), $member->full_name))
			->build('admin/form', $this->data);
	}

	/**
	 * Show a client preview
	 * @access	public
	...
	{
		$data->client = $this->ion_auth->get_user($id);
	 ...
	 * Activate a client
	 * @access public
	 * @param int $id The ID of the client to activate
	 * @return void
	 */
	public function activate()
	{
		...
		{
			$this->session->set_flashdata('error', $this->lang->line('client_activate_error'));
			redirect('admin/clients');
		}
		...
		$this->session->set_flashdata('success', sprintf($this->lang->line('client_activate_success'), $activated, $to_activate));
	...
	public function delete($id = 0)
	{
		...
		if (!empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				{
					$this->session->set_flashdata('notice', $this->lang->line('client_delete_self_error'));
					continue;
				}
		...
		// The array of id's to delete is empty
			...
			$this->session->set_flashdata('error', $this->lang->line('client_mass_delete_error'));
	...
	public function _username_check($username)
	{
		...
		{
			$this->form_validation->set_message('_username_check', $this->lang->line('client_error_username'));
			return FALSE;
		}
		else
		{
	...
	public function _email_check($email)
	{
		...
		{
			$this->form_validation->set_message('_email_check', $this->lang->line('client_error_email'));
			return FALSE;
		}
		else
		{
