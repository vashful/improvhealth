 ...
 *
 */
class Filariasis_m extends MY_Model
{
	/**
	...
	public function insert($input = array())
	{
                ...
                {                
                  $data =array(
                        ...
                        'mf' => ($input['mf'])?1:0,
                        'given_mda' => ($input['given_mda'])?1:0,
                        'adeno' => ($input['adeno'])?1:0,
                        'date_added' => $input['date_given']
                        );
                
                  $this->db->insert('filariasis', $data); 

                  $filariasis_id = $this->db->insert_id();
                }
                else
                {
                  return false;
...
//               print_r($input);die();
                  ...
                  $this->db->where('id', $id);
                  $this->db->update('filariasis', $data); 
	...
	{
		$this->db->delete('filariasis', array('id' => $id)); 								
		return true;
	}
    ...
    $this->db->from('filariasis');
    $this->db->join('clients', 'clients.id = filariasis.client_id');
    ...
    public function count_rows_on_table($table = 'filariasis', $field = 'id' ,$value = 0)
    {
    ...
    {
        $this->db->select('*, filariasis.id as filariasis_id');
        $this->db->from('filariasis');
        $this->db->join('clients', 'clients.id = filariasis.client_id');
    ...
    public function get_filariasis_details($id = null)
    {
          $this->db->select('*, filariasis.id as filariasis_id');
          $this->db->from('filariasis');
      		$this->db->where('filariasis.id', $id);
