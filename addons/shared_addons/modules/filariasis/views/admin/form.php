...
<section class="title">
    <?php if ($sc->method_action == 'add'): ?>
    		<h4>Add <em>"<?php echo $client->last_name; ?>, <?php echo $client->first_name; ?>"</em> as a Filariasis Client</h4>
    		<?php echo form_open(uri_string(), 'class="crud" autocomplete="off"'); ?>
       	<?php else: ?>
    		<h4><?php echo sprintf('Edit Filariasis Client "%s"', $client->first_name);?></h4>
	...
	<div class="tabs">
		...
		<div class="form_inputs" id="client-vitamin-tab">
          ...
          <li class="even">
						...
						<?php echo form_checkbox('mf', '1', $sc->mf ? TRUE : FALSE, 'id=mf'); ?> <span>MF in the slides found (+)</span></br>
						<label></label><?php echo form_checkbox('given_mda', '1', $sc->given_mda ? TRUE : FALSE, 'id=given_mda'); ?> <span>Person given MDA</span></br>
						<label></label><?php echo form_checkbox('adeno', '1', $sc->adeno ? TRUE : FALSE, 'id=adeno'); ?> <span>Adenolymphangitis Case</span></br>
		<li>
