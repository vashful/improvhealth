...
class Admin extends Admin_Controller
{
  ...
  /**
	...
	public function __construct()
	{
		...
		$this->load->model(array('malaria_m','clients/clients_m', 'clients/clients_region_m', 'clients/clients_province_m', 'clients/clients_city_m', 'clients/clients_barangay_m'));
		$this->load->library('form_validation');
        $this->load->library('fpdf'); // Load FPDF library
		$this->lang->load(array('malaria','clients/clients'));     
	...
	public function index()
	{
		...
		redirect('admin/malaria/my_list');
    ...
    $base_where = $this->input->post('f_barangays') ? $base_where + array('barangay' => $this->input->post('f_barangays')) : $base_where;
       ...
       $this->session->set_userdata('base_where',$base_where);
		...
		$pagination = create_pagination('admin/malaria/index', $this->malaria_m->count_by($base_where));
		...
		$clients = $this->malaria_m->get_results($pagination['limit'],$base_where);
        
        $this->data->totalitems = $this->malaria_m->count_by($base_where);
		...
		$this->template
			...
			->append_metadata(js('admin/filter.js'))
                        ->set_partial('malaria-list', 'admin/tables/clients');
	...
	public function my_list()
	{
		...
		$selected_client or redirect('admin/malaria');
		
    //determine active param
		$base_where['client_id'] = $this->session->userdata('selected_client_id');
		
				// Create pagination links
		$pagination = create_pagination('admin/malaria/my_list', $this->malaria_m->count_by($base_where));
    ...
    $this->data->selected_client = & $selected_client;
		...
		$this->template
			...
			->set('pagination', $pagination)
			->set('clients', $clients)      
      ->set_partial('malaria-my_list', 'admin/tables/my_list');
  ...
  public function add_client()
	{		
                $this->session->set_flashdata('warning', 'Search and Select a Client to be Added to "Malaria" Or <a href="admin/clients/add" class="success-link">Register</a> a New Client.');
                $this->session->set_userdata('current_action', 'malaria'); 
    ...
    {
     $this->session->set_flashdata('warning', 'Please Add a New Consultation for this Client before Adding a Record for Malaria');
     redirect('admin/consultations/add/'.$client_id);
    } 
    
    $client = $this->clients_m->get($client_id);
    $client or redirect('admin/malaria');
    ...
    if ($_POST)
      ...
      $secure_post['date_given_treatment'] = $date_given_treatment;*/
          		...
          		{ 
                                if($this->malaria_m->insert($secure_post))
                                {
                                  $this->session->set_flashdata('success', 'New Malaria cient added');
                                ...
                                {
                                $this->session->set_flashdata('error', 'Error adding Malaria client/ No data has been entered.');
                                redirect("admin/malaria/add/$client_id");
                                }
  ...
  { 
    ...
    $count = $this->malaria_m->check_record($client_id);
    if($count > 0)
    {
        $this->session->set_flashdata('warning', 'Please Select on the List or '."<a href='admin/malaria/add/$client_id' class='add'>Add</a>".' New Record Under Malaria for this Client');
    ...
    {
        $this->session->set_flashdata('error', 'No Record Under Malaria Found for this Client. Click '."<a href='admin/malaria/add/$client_id' class='add'>here</a>".' to Add New Record for this Client Under Malaria');
        $this->session->set_userdata('selected_client_id',$client_id);
        $this->session->set_userdata('set', $type);
        redirect("admin/malaria/my_list");
    }
	...
	{
	  $sc = $this->malaria_m->get_malaria_details($id);
    ...
    if ($_POST)
      ...
      $secure_post['date_given'] = $date_given;
          		...
          		if ($this->form_validation->run() !== FALSE)
          		{ 
                                if($this->malaria_m->update($id, $secure_post))
                                {
                                  $this->session->set_flashdata('success', 'Malaria Client successfully updated');
                                  if($this->session->userdata('selected_client_id'))
                                  redirect('admin/malaria/my_list');
                                  else
                                  redirect('admin/malaria');
                                ...
                                {
                                  $this->session->set_flashdata('error', 'Error updating Malaria Client/No Data has been entered.');
                                  redirect("admin/malaria/edit/$id");
                                }
	...
	public function delete($id = 0)
	{
    	...
    	echo $ids;die();
    		if (!empty($ids))
    		{
    			...
    			if ($to_delete > 0)
    			{
    				$this->session->set_flashdata('success', sprintf($this->lang->line('malaria_mass_delete_success'), $deleted, $to_delete));
    			}
    		}
    		// The array of id's to delete is empty
    		else
    			$this->session->set_flashdata('error', $this->lang->line('malaria.delete_error'));
    }
    else
    {
        $this->malaria_m->delete($id)
    			? $this->session->set_flashdata('success', lang('malaria.delete_success'))
    			: $this->session->set_flashdata('error', lang('malaria.delete_error'));
		}
	  if($this->session->userdata('selected_client_id'))
      redirect('admin/malaria/my_list');
    else
