...
<section class="title">
    <?php if ($sc->method_action == 'add'): ?>
    		<h4>Add <em>"<?php echo $client->last_name; ?>, <?php echo $client->first_name; ?>"</em> as a Schistosomiasis Client</h4>
    		<?php echo form_open(uri_string(), 'class="crud" autocomplete="off"'); ?>
       	<?php else: ?>
    		<h4><?php echo sprintf('Edit Schistosomiasis Client "%s"', $client->first_name);?></h4>
...
<section class="item">
	...
	<div class="tabs">
		...
		<ul class="tab-menu">
			<li><a href="#client-vitamin-tab"><span>Diagnosis</span></a></li>
		...
		<div class="form_inputs" id="client-vitamin-tab">
          ...
          <li class="even">
						<label>Diagnosis</label><?php echo form_checkbox('positive', '1', $sc->positive ? TRUE : FALSE, 'id=positive'); ?> <span>Positive case</span></br>
						<?php echo form_checkbox('treated', '1', $sc->treated ? TRUE : FALSE, 'id=treated'); ?> <span>Case treated</span></br>
						<label></label><?php echo form_checkbox('referred', '1', $sc->referred ? TRUE : FALSE, 'id=referred'); ?> <span>Case referred to hospital facilities</span></br>
						<li><label>Case examined</label><div class="input">
						<?php echo form_dropdown('examined', $case_examined_list, $sc->examined, 'id="examined" class="chzn"'); ?>
			</li>
		<li>
