...
class Admin extends Admin_Controller
{
  ...
  /**
	...
	public function __construct()
	{
		...
		$this->load->model(array('sc_m','clients/clients_m', 'clients/clients_region_m', 'clients/clients_province_m', 'clients/clients_city_m', 'clients/clients_barangay_m'));
		$this->load->library('form_validation');
        $this->load->library('fpdf'); // Load FPDF library
		$this->lang->load(array('sc','clients/clients'));     
		//$this->output->enable_profiler(TRUE);
		
		$this->data->methods = array();
		if ($methods = $this->sc_m->get_method())
		{
	...
	public function index()
	{
		...
		redirect('admin/sick_children/my_list');
    ...
    $base_where = $this->input->post('f_barangays') ? $base_where + array('barangay' => $this->input->post('f_barangays')) : $base_where;
       ...
       $this->session->set_userdata('base_where',$base_where);
        ...
        $this->data->totalitems = $this->sc_m->count_by($base_where);
		...
		$this->template
			...
			->append_metadata(js('admin/filter.js'))
                        ->set_partial('sc-list', 'admin/tables/clients');
	...
	public function my_list()
	{
		...
		$selected_client or redirect('admin/sick_children');
		
    //determine active param
		$base_where['client_id'] = $this->session->userdata('selected_client_id');
		
				// Create pagination links
		$pagination = create_pagination('admin/sick_children/my_list', $this->sc_m->count_by($base_where));
    ...
    $this->data->selected_client = & $selected_client;
		...
		$this->template
			...
			->set('pagination', $pagination)
			->set('clients', $clients)      
      ->set_partial('sc-my_list', 'admin/tables/my_list');
  ...
  public function add_client()
	{		
                $this->session->set_flashdata('warning', 'Search and Select a Client to be Added to "Sick Children" Or <a href="admin/clients/add" class="success-link">Register</a> a New Client.');
                $this->session->set_userdata('current_action', 'sc'); 
    ...
    {
     $this->session->set_flashdata('warning', 'Please Add New Consultation for this Client before Adding Record for Sick Children');
     redirect('admin/consultations/add/'.$client_id);
    } 
    
    $client = $this->clients_m->get($client_id);
    $client or redirect('admin/sick_children');
    ...
    if ($this->input->post('date_given'))
		...
		if ($this->input->post('date_started'))
		{
			$date_started = strtotime($this->input->post('date_started'));  
		}
		else
		{
			$date_started = '';  
		...
		{
			$date_completed = strtotime($this->input->post('date_completed'));  
		}
		else
		{
			$date_completed = '';   
		...
		{
			$ort = strtotime($this->input->post('ort'));  
		}
		else
		{
			$ort = '';   
		...
		if ($this->input->post('ors'))
		{
			$ors = strtotime($this->input->post('ors'));  
		}
		else
		{
			$ors = '';   
		...
		if ($this->input->post('ors_zinc'))
		{
			$ors_zinc = strtotime($this->input->post('ors_zinc'));  
		}
		else
		{
			$ors_zinc = '';   
		...
		if ($this->input->post('date_given_treatment'))
		{
			$date_given_treatment = strtotime($this->input->post('date_given_treatment'));  
		}
		else
		{
			$date_given_treatment = '';   
		}
    ...
    if ($_POST)
      ...
      $secure_post['date_started'] = $date_started;
      $secure_post['date_completed'] = $date_completed;
      $secure_post['ort'] = $ort;
      $secure_post['ors'] = $ors;
      $secure_post['ors_zinc'] = $ors_zinc;
      $secure_post['date_given_treatment'] = $date_given_treatment;
      

          		if ($this->form_validation->run() !== FALSE)
          		{ 
                                if($this->sc_m->insert($secure_post))
                                {
                                  $this->session->set_flashdata('success', 'New Children Added');
                                  $this->session->set_userdata('consultation_id', 0);
                                  if($this->session->userdata('selected_client_id'))
                                    redirect('admin/sick_children/my_list');
                                  else
                                    redirect('admin/sick_children');
                                }
                                else 
                                {
                                $this->session->set_flashdata('error', 'Error Adding Children/No Data has been entered.');
                                redirect("admin/sick_children/add/$client_id");
                                }
    ...
    $sc->date_started = $date_started;
    $sc->date_completed = $date_completed;
    $sc->ort = $ort;
    $sc->ors = $ors;
    $sc->ors_zinc = $ors_zinc;
    $sc->date_given_treatment = $date_given_treatment;
  ...
  { 
    ...
    $count = $this->sc_m->check_record($client_id);
    if($count > 0)
    {
        $this->session->set_flashdata('warning', 'Please Select on the List or '."<a href='admin/sick_children/add/$client_id' class='add'>Add</a>".' New Record Under Sick Children for this Client');
    ...
    {
        $this->session->set_flashdata('error', 'No Record Under Sick Children Found for this Client. Click '."<a href='admin/sick_children/add/$client_id' class='add'>here</a>".' to Add New Record for this Client Under Sick Children');
        $this->session->set_userdata('selected_client_id',$client_id);
        $this->session->set_userdata('set', $type);
        redirect("admin/sick_children/my_list");
    }
	...
	{
	  $sc = $this->sc_m->get_sc_details($id);
    ...
    if ($this->input->post('date_given'))
		...
		{
			$date_given = '';  
			$sc->date_given = $sc->date_given ? date('Y-m-d', $sc->date_given) : ''; 
		}
		...
		{
			$date_started = '';  
			$sc->date_started = $sc->date_started ? date('Y-m-d', $sc->date_started) : ''; 
		}
		...
		{
			$date_completed = '';   
			$sc->date_completed = $sc->date_completed ? date('Y-m-d', $sc->date_completed) : ''; 
		}
		...
		{
			$ort = '';  
      $sc->ort = $sc->ort ? date('Y-m-d', $sc->ort) : ''; 
		}
		...
		{
			$ors = '';   
			$sc->ors = $sc->ors ? date('Y-m-d', $sc->ors) : ''; 
		}
		...
		{
			$ors_zinc = '';   
			$sc->ors_zinc = $sc->ors_zinc ? date('Y-m-d', $sc->ors_zinc) : ''; 
		}
		...
		{
			$date_given_treatment = '';  
      $sc->date_given_treatment = $sc->date_given_treatment ? date('Y-m-d', $sc->date_given_treatment) : ''; 
		}
		
		if ($this->input->post('six_months'))
		{
			$six = $this->input->post('six_months');  
		}
		else
		{
			$six = '0';   
		...
		if ($this->input->post('twelve_months'))
		{
			$twelve = $this->input->post('twelve_months');  
		}
		else
		{
			$twelve = '0';   
		...
		if ($this->input->post('sixty_months'))
		{
			$sixty = $this->input->post('sixty_months');  
		}
		else
		{
			$sixty = '0';   
    ...
    if ($_POST)
      ...
      $secure_post['six_months'] = $six;
      $secure_post['twelve_months'] = $twelve;
      $secure_post['sixty_months'] = $sixty;
                
          		if ($this->form_validation->run() !== FALSE)
          		{ 
                                if($this->sc_m->update($id, $secure_post))
                                {
                                  $this->session->set_flashdata('success', 'Children successfully updated');
                                  if($this->session->userdata('selected_client_id'))
                                  redirect('admin/sick_children/my_list');
                                  else
                                  redirect('admin/sick_children');
                                ...
                                else 
                                {
                                  ...
                                  redirect("admin/sick_children/edit/$id");
                                }
	...
	public function delete($id = 0)
	{
        ...
        $ids = $this->input->post('action_to');
    		...
    		if (!empty($ids))
    		{
    			...
    			if ($to_delete > 0)
    			{
    				$this->session->set_flashdata('success', sprintf($this->lang->line('sc_mass_delete_success'), $deleted, $to_delete));
    			}
    		}
    		// The array of id's to delete is empty
    		else
    			$this->session->set_flashdata('error', $this->lang->line('sc.delete_error'));
    }
    else
    {
        $this->sc_m->delete($id)
    			? $this->session->set_flashdata('success', lang('sc.delete_success'))
    			: $this->session->set_flashdata('error', lang('sc.delete_error'));
		}
	  if($this->session->userdata('selected_client_id'))
      redirect('admin/sick_children/my_list');
    else
