...
class Admin extends Admin_Controller
{
  ...
  /**
	...
	public function __construct()
	{
		...
		$this->load->model(array('leprosy_m','clients/clients_m', 'clients/clients_region_m', 'clients/clients_province_m', 'clients/clients_city_m', 'clients/clients_barangay_m'));
		$this->load->library('form_validation');
        $this->load->library('fpdf'); // Load FPDF library
		$this->lang->load(array('leprosy','clients/clients'));     
		...
		$this->data->diagnosis_list = array(
						      ''  => '-- Please Select --',
                  'A'  => 'Leprosy case',
                  'B'  => 'Newly detected Leprosy case',
                  'C'    => 'Newly detected case with Grade 2 disability',
                  'D'    => 'Case cured'
                );
	...
	public function index()
	{
		...
		redirect('admin/leprosy/my_list');
    ...
    $base_where = $this->input->post('f_barangays') ? $base_where + array('barangay' => $this->input->post('f_barangays')) : $base_where;
       ...
       $this->session->set_userdata('base_where',$base_where);
		...
		$pagination = create_pagination('admin/leprosy/index', $this->leprosy_m->count_by($base_where));
		...
		$clients = $this->leprosy_m->get_results($pagination['limit'],$base_where);
        
        $this->data->totalitems = $this->leprosy_m->count_by($base_where);
		...
		$this->template
			...
			->append_metadata(js('admin/filter.js'))
                        ->set_partial('leprosy-list', 'admin/tables/clients');
	...
	public function my_list()
	{
		...
		$selected_client or redirect('admin/leprosy');
		
    //determine active param
		$base_where['client_id'] = $this->session->userdata('selected_client_id');
		
				// Create pagination links
		$pagination = create_pagination('admin/leprosy/my_list', $this->leprosy_m->count_by($base_where));
    ...
    $this->data->selected_client = & $selected_client;
		...
		$this->template
			...
			->set('pagination', $pagination)
			->set('clients', $clients)      
      ->set_partial('leprosy-my_list', 'admin/tables/my_list');
  ...
  public function add_client()
	{		
                $this->session->set_flashdata('warning', 'Search and Select a Client to be Added to "Leprosy" Or <a href="admin/clients/add" class="success-link">Register</a> a New Client.');
                $this->session->set_userdata('current_action', 'leprosy'); 
    ...
    {
     $this->session->set_flashdata('warning', 'Please Add a New Consultation for this Client before Adding a Record for Leprosy');
     redirect('admin/consultations/add/'.$client_id);
    } 
    
    $client = $this->clients_m->get($client_id);
    $client or redirect('admin/leprosy');
    ...
    if ($_POST)
      ...
      $secure_post['date_given_treatment'] = $date_given_treatment;*/
          		...
          		{ 
                                if($this->leprosy_m->insert($secure_post))
                                {
                                  $this->session->set_flashdata('success', 'New leprosy cient added');
                                ...
                                {
                                $this->session->set_flashdata('error', 'Error adding leprosy client/ No data has been entered.');
                                redirect("admin/leprosy/add/$client_id");
                                }
  ...
  { 
    ...
    $count = $this->leprosy_m->check_record($client_id);
    if($count > 0)
    {
        $this->session->set_flashdata('warning', 'Please Select on the List or '."<a href='admin/leprosy/add/$client_id' class='add'>Add</a>".' New Record Under Leprosy for this Client');
    ...
    {
        $this->session->set_flashdata('error', 'No Record Under Leprosy Found for this Client. Click '."<a href='admin/leprosy/add/$client_id' class='add'>here</a>".' to Add New Record for this Client Under Leprosy');
        $this->session->set_userdata('selected_client_id',$client_id);
        $this->session->set_userdata('set', $type);
        redirect("admin/leprosy/my_list");
    }
	...
	{
	  $sc = $this->leprosy_m->get_leprosy_details($id);
    ...
    if ($_POST)
      ...
      // Set the full date of birth
       $secure_post['date_given'] = $date_given;
          		...
          		if ($this->form_validation->run() !== FALSE)
          		{ 
                                if($this->leprosy_m->update($id, $secure_post))
                                {
                                  $this->session->set_flashdata('success', 'Leprosy Client successfully updated');
                                  if($this->session->userdata('selected_client_id'))
                                  redirect('admin/leprosy/my_list');
                                  else
                                  redirect('admin/leprosy');
                                ...
                                {
                                  $this->session->set_flashdata('error', 'Error updating Leprosy Client/No Data has been entered.');
                                  redirect("admin/leprosy/edit/$id");
                                }
	...
	public function delete($id = 0)
	{
    	...
    	echo $ids;die();
    		if (!empty($ids))
    		{
    			...
    			if ($to_delete > 0)
    			{
    				$this->session->set_flashdata('success', sprintf($this->lang->line('leprosy_mass_delete_success'), $deleted, $to_delete));
    			}
    		}
    		// The array of id's to delete is empty
    		else
    			$this->session->set_flashdata('error', $this->lang->line('leprosy.delete_error'));
    }
    else
    {
        $this->leprosy_m->delete($id)
    			? $this->session->set_flashdata('success', lang('leprosy.delete_success'))
    			: $this->session->set_flashdata('error', lang('leprosy.delete_error'));
		}
	  if($this->session->userdata('selected_client_id'))
      redirect('admin/leprosy/my_list');
    else
